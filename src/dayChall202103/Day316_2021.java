...
class SolutionDay316 {
    private int profit(List<Integer> pricesList, int buyDay, int sellDay, int fee) {
        if (pricesList == null) return 0;
        return pricesList.get(sellDay) - pricesList.get(buyDay) - fee;
    }

    // Take off leading downhill
    private List<Integer> profitFilter0(int[] prices) {
        if (prices == null) return null;
        List<Integer> result = new ArrayList<>();
        if (prices.length < 2) return result;
        boolean downHillHead = true;
        for (int i = 1; i < prices.length; i++) {
            if (downHillHead && prices[i] > prices[i - 1]) {
                downHillHead = false;
                result.add(prices[i - 1]);
            }
            if (!downHillHead)
                result.add(prices[i]);
        }
    ...
    // Throw out intermediates
    private List<Integer> profitFilter1(List<Integer> pricesList) {
        if (pricesList == null) return null;
        int buyDay = 0;
        int sellDay = 1;
        List<Integer> result = new ArrayList<>();
        if (pricesList.size() < 2) return result;
        for (int i = 1; i < pricesList.size(); i++) {
            if (pricesList.get(i) < pricesList.get(i - 1)) {
                sellDay = i - 1;
                if (buyDay < sellDay) {
                    result.add(pricesList.get(buyDay));
                    result.add(pricesList.get(sellDay));
                }
                buyDay = i;
            } else if (pricesList.get(i) > pricesList.get(i - 1)) {
                if (buyDay > sellDay) {
                    buyDay = i - 1;
                }
                sellDay = i;
    ...
    // Maximize profits
    private List<Integer> profitFilter2(List<Integer> pricesList, int fee) {
        if (pricesList == null) return null;
        if (pricesList.size() < 4)
            return pricesList;
        int buyDay1 = 0, sellDay1 = 1;
        int buyDay2, sellDay2 = 1;
        List<Integer> result = new ArrayList<>();
        for (int i = 2; i < pricesList.size(); i++, i++) {
            buyDay2 = i;
            sellDay2 = i + 1;
            if (profit(pricesList, buyDay1, sellDay2, fee) <=
                    profit(pricesList, buyDay1, sellDay1, fee) +
                            profit(pricesList, buyDay2, sellDay2, fee)) {
                result.add(pricesList.get(buyDay1));
                result.add(pricesList.get(sellDay1));
                buyDay1 = buyDay2;
            }
            sellDay1 = sellDay2;
        }
        result.add(pricesList.get(buyDay1));
        result.add(pricesList.get(sellDay2));
        return result;
    }

    // No Negatives
    private List<Integer> profitFilter3(List<Integer> pricesList, int fee) {
        if (pricesList == null) return null;
        List<Integer> result = new ArrayList<>();
        for (int i = 0; i < pricesList.size(); i++, i++) {
            if (profit(pricesList, i, i + 1, fee) > 0) {
                result.add(pricesList.get(i));
                result.add(pricesList.get(i+1));
            }
        }
        return result;
    }
    ...
    public int maxProfit(int[] prices, int fee) {
        ...
        System.out.println(prices.length+" "+ Arrays.toString(prices));
        List<Integer> pricesList = profitFilter0(prices);
        System.out.println(pricesList.size()+" "+pricesList);
        pricesList = profitFilter1(pricesList);
        System.out.println(pricesList.size()+" "+pricesList);
        pricesList = profitFilter2(pricesList, fee);
        System.out.println(pricesList.size()+" "+pricesList);
        pricesList = profitFilter3(pricesList, fee);
        System.out.println(pricesList.size()+" "+pricesList);

        int maxProfit = 0;
        int stop = pricesList.size();
        for (int i = 0; i < stop; i++, i++) {
            maxProfit += profit(pricesList, i, i + 1, fee);
        }
        return maxProfit;
    }
}
...
//        AnswerDay316 solution = new AnswerDay316();
        ...
        // Sumpin 1
        input1 = new int[]{11, 19};
        input2 = 5;
        result = solution.maxProfit(input1, input2);
        System.out.println("3 == " + result);

        // Sumpin 2
        input1 = new int[]{11, 15, 19};
        input2 = 5;
        result = solution.maxProfit(input1, input2);
        System.out.println("3 == " + result);

        // Sumpin 3
        input1 = new int[]{11, 10, 12, 11, 13, 12, 14, 13, 15};
        input2 = 3;
        result = solution.maxProfit(input1, input2);
        System.out.println("2 == " + result);

        // Sumpin 4
        input1 = new int[]{20, 11, 5, 11, 9, 15, 19};
        input2 = 5;
        result = solution.maxProfit(input1, input2);
        System.out.println("9 == " + result);

        // Example 1
        input1 = new int[]{1, 3, 2, 8, 4, 9};
        input2 = 2;
        result = solution.maxProfit(input1, input2);
        System.out.println("8 == " + result);

        // Example 2
        input1 = new int[]{1, 3, 7, 5, 10, 3};
        input2 = 3;
        result = solution.maxProfit(input1, input2);
        System.out.println("6 == " + result);

        // Test Case 1
        input1 = new int[]{1, 5, 9};
        input2 = 2;
        result = solution.maxProfit(input1, input2);
        System.out.println("6 == " + result);

        // Test Case 2
        input1 = new int[]{9, 8, 7, 1, 2};
        input2 = 3;
        result = solution.maxProfit(input1, input2);
        ...
        // Test Case 4
        input1 = new int[]{1,2,1,5,3,5,5,4,1,5};
        input2 = 0;
        result = solution.maxProfit(input1, input2);
